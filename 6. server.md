...
**Think of it like:**
- ESP32 sends: "Temperature: 25°C\n"
- Parser collects characters until it sees `\n`
- Then gives us the complete line: "Temperature: 25°C"
...
- The message includes an emoji ✅ to show success
...
- Example output: "🌤️ Weather Station: Temperature: 25°C"
...
- `'/screen/:view'` = URL with a parameter
  ...
  - Example URLs:
    - `http://localhost:3000/screen/TEMP` → view = "TEMP"
    - `http://localhost:3000/screen/HUMIDITY` → view = "HUMIDITY"
...
1. **Server starts** → Opens port 3000 and connects to ESP32 on COM8
2. **Browser visits** `http://localhost:3000` → Server sends index.html
3. **User clicks button** → Browser sends request to `/location` or `/screen/TEMP`
4. **Server receives request** → Processes it and sends command to ESP32
5. **ESP32 receives command** → Changes location or screen view
6. **ESP32 sends data back** → Parser reads it line-by-line
7. **Server logs data** → You see it in the terminal
